...
###Note: Belo chunk is the right pseudo-alignment chunk##### *Running
kallisto quantification command*: tells you how much of each transcript
...
Giving error that "--paired" not valid-\> Kallisto automatically
recognizes paired files.

*Below chunk is wrong, it does not tell Kallisto I have paired-end
reads. Do not use. Re-did on chunk above to correct this.*
...
--- 
```{r}
colnames(counts) #counts-> is edited to have modified names of samples, ex. 26_1, 26_2, ect.
...
Now I have to rename the rows of the counts matrix to match the name of
the genes given in the RNA.FNA file. Need biostrings package.
...
These names are really long, not practical to use them for DESeq
analysis, must find other method to display gene names during downstream
analysis. Ask Steven about this.

------------------------------------------------------------------------

Re-doing the colnames bc what I did previously did not work with DESeq,
was identifying each name as 12 different conditions and only one
dataset for each, that is wrong.
...
Above chunk gives warning that DESeq is not using the control as the
reference (for non-treated counts), have to tell deseq to use control as
reference to get accurate analysis.
...
**8. Reading in count matrix** \*Switching to R language and displaying
first few rows.
...
Getting DEG based on Desication. Run this table using 2 control, 2 26
deg, and 2 30 deg.
